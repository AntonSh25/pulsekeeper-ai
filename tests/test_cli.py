...
def test_telegram_run_handles_keyboard_interrupt_as_graceful_shutdown(monkeypatch, tmp_path):
    config_path = tmp_path / "config.toml"
    config_path.write_text(
        """
[storage]
...
""".strip(),
    ...
    monkeypatch.setattr("pulsekeeper.cli._telegram_run_async", raise_keyboard_interrupt)

    result = runner.invoke(
        app,
        ["telegram-run", "--config", str(config_path)],
    )
